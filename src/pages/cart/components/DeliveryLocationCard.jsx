  ...
  return (
    <div>
      ...
      <div className="flex justify-between items-start p-3 bg-gray-200 text-black rounded-lg shadow-sm my-3">
        <div className="flex gap-2">
          ...
          <div className="flex flex-col gap-1">
            <span className="font-semibold text-sm pb-1">
              Delivery to:{currentDeliveryLocation?.tag}
            </span>
            <span className="font-extralight text-xs">
              {currentDeliveryLocation
                ? currentDeliveryLocation.house
                : "No address selected"}
            </span>
            {currentDeliveryLocation && (
              <>
        ...
        <span
          className="text-xs text-main-color font-bold text-end cursor-pointer"
              ...
              <ul className="space-y-3">
                {addresses.map((location, index) => (
                  <li
                    ...
                    className={`cursor-pointer p-3 bg-gray-100 rounded-lg shadow hover:bg-gray-200 dark:hover:bg-main-color/10 ${
                      currentDeliveryLocation?.id === location.id
                        ? "border border-main-color"
                        : ""
                    }`}
                  >
                    <span className="font-medium">{location.tag} - {location?.address}</span>
